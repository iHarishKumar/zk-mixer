...
contract IncrementalMerkleTree {
  ...
  mapping(uint256 => bytes32) public s_root;
  uint32 public constant ROOT_HISTORY_SIZE = 30;
  uint32 public s_currentRootIndex;

  error IncrementalMerkleTree_DepthCannotBeZer0();
  error IncrementalMerkleTree_DepthCannotBeGreaterThan32(uint256 _depth);
  error IncrementalMerkleTree_LeafIndexCannotBeGreaterThanMaxIndex();
  error IncrementalMerkleTree_MerkleTreeFull(uint32 _nextLeafIndex);

  uint32 public s_nextLeafIndex;

  bytes32 public ini_zeroNode = bytes32(uint256(keccak256("harish")) % uint256(Field.PRIME));

  bytes32[] public s_zeroHashes;
  mapping(uint32 => bytes32) public s_cachedSubtrees;

  constructor(uint32 _depth, Poseidon2 _hasher) {
    if(_depth == 0) {
      revert IncrementalMerkleTree_DepthCannotBeZer0();
    }
    if(_depth >= 32) {
      revert IncrementalMerkleTree_DepthCannotBeGreaterThan32(_depth);
    }
    i_depth = _depth;
    i_hasher = _hasher;
    ...
    s_zeroHashes = new bytes32[](i_depth);
    s_root[0] = zeros(_depth); // store the ID 0 root as the zero depth zero
  }
  ...
  function _insert(bytes32 _leaf) internal returns(uint32) {
    ...
    // Check that the index of the leaf being addedd is within the maximum index i.e (2^depth - 1) as the index starts with 0
    if(_nextLeafIndex > (uint32(2)**i_depth)) {
      revert IncrementalMerkleTree_MerkleTreeFull(_nextLeafIndex);
    }
    ...
    for(uint32 i = 0; i < i_depth; i++) {
      if(currentIndex % 2 == 0) {
        // - If it is even, then we need to put it on the left of the hash and a zero on the right and store the result as a cached subtree
        left = currentHash;
        right = zeros(i);
        s_cachedSubtrees[i] = currentHash;
      } else {
        // - If it is odd, then we need to put it on the right and a cached subtree on the left
    ...
    s_root[newRootIndex] = currentHash;
...
function zeros(uint256 i) public pure returns (bytes32) {
        ...
        else revert IncrementalMerkleTree_DepthCannotBeGreaterThan32(i);
    }
  ...
  function isKnownRoot(bytes32 _root) public returns(bool) {
    ...
    // check if the _root matches the one in s_root
